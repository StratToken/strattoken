...
import PrivatesaleToken_json    from '../../build/contracts/PrivatesaleToken';
import TokenManager_json    from '../../build/contracts/TokenManager';
import Const                from './constants';

const PrivatesaleToken = Truffle(PrivatesaleToken_json);
const TokenManager = Truffle(TokenManager_json);
const web3 = window.web3;


if(web3) {
  PrivatesaleToken.setProvider(web3.currentProvider);
...
const API = {
  ...
  getTokenInfo: tokenAddress => new Promise((resolve, reject) => {
    try {
      resolve(PrivatesaleToken.at(tokenAddress));
    } catch(err) {
  ...
  setCrowdsaleManager: (tokenAddr, mgrAddr, crowdsaleAddress) => {
    const m = TokenManager.at(mgrAddr);
    return m.tokenSetCrowdsaleManager(
      tokenAddr, crowdsaleAddress, 
      {gas: 500000, from: web3.eth.accounts[0]}
    );
  },

  SetEscrow: (mgrAddr, tokens) => {
    const m = TokenManager.at(mgrAddr);
    return m.setEscrowBalance(
      tokens, 
      {gas: 500000, from: web3.eth.accounts[0]}
    );
  },
